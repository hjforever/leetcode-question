...
//number of new customers arriving just before the iᵗʰ rotation (0-indexed). This means 
...
//order to get all the customers down safely. Note that if there are currently more 
//than four customers waiting at the wheel, only four will board the gondola, and 
//the rest will wait for the next rotation. 
//
...
// Related Topics 数组 模拟 👍 9 👎 0
  ...
  public class Q1599MaximumProfitOfOperatingACentennialWheel{
      public static void main(String[] args) {
           Solution solution = new Q1599MaximumProfitOfOperatingACentennialWheel().new Solution();
      }
...
class Solution {
          public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
              ...
              // 待登舱用户,历史登舱过的用户数
              int todoCount = 0, historyCount = 0;
              // 最大利润
              int maxProfit = 0;
              int ans = -1;
              for (int step = 1; step <= customers.length || todoCount > 0; step++) {
                  if (step <= customers.length) {
                      todoCount += customers[step - 1];
                  }
                  ...
                  if (todoCount > 4) {
                      ...
                      historyCount += todoCount;
                      todoCount = 0;
                  }

                  int profit = historyCount * boardingCost - step * runningCost;
