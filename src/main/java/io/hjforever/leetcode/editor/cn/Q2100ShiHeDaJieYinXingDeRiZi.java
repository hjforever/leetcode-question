...
// 
// Related Topics 数组 动态规划 前缀和 👍 22 👎 0
...
public class Q2100ShiHeDaJieYinXingDeRiZi {
    public static void main(String[] args) {
        Solution solution = new Q2100ShiHeDaJieYinXingDeRiZi().new Solution();
    ...
    class Solution {
        public List<Integer> goodDaysToRobBank(int[] security, int time) {
            ...
            //双指针
            if (security == null || security.length <= 2 * time) return res;
            int n = security.length;
            int[] leftDp = new int[security.length];
            int[] rightDp = new int[security.length];
