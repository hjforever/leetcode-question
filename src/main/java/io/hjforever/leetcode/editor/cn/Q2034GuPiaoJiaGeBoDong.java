...
// 
// Related Topics 设计 哈希表 数据流 有序集合 堆（优先队列） 👍 21 👎 0
...
public class Q2034GuPiaoJiaGeBoDong {
    ...
    class StockPrice {
        ...
        public void update(int timestamp, int price) {
            ...
            int pre = timePrices.getOrDefault(timestamp, 0);
            timePrices.put(timestamp, price);
            if (pre > 0) {
                //移除前一个价格
                int count = prices.getOrDefault(pre, 0);
                if (count > 1) {
                    prices.put(pre, count - 1);
                } else {
                    prices.remove(pre);
                }
            }
