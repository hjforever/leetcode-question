...
public class Q806XieZiFuChuanXuYaoDeXingShu {
    public static void main(String[] args) {
        Solution solution = new Q806XieZiFuChuanXuYaoDeXingShu().new Solution();
    }
    ...
    class Solution {
        public int[] numberOfLines(int[] widths, String s) {
            ...
            for (int i = 0; i < s.length(); i++) {
                int chWith = widths[s.charAt(i) - 'a'];
                if (lineWith + chWith > M) {
                    lineWith = chWith;
                    line++;
                } else {
                    lineWith += chWith;
