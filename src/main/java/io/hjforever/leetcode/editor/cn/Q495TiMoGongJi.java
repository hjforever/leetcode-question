...
public class Q495TiMoGongJi {
    public static void main(String[] args) {
        Solution solution = new Q495TiMoGongJi().new Solution();
    }
    ...
    class Solution {
        public int findPoisonedDuration(int[] timeSeries, int duration) {
            int posioneTime = 0;
            int pt = timeSeries[0] + duration;
            for (int i = 1; i < timeSeries.length; i++) {
                int time = timeSeries[i];
                if (time > pt) {
                    posioneTime += duration;
                } else {
                    posioneTime += time - timeSeries[i - 1];
                }
                pt = time + duration;
            }
            return posioneTime + duration;
        }
    }
