...
public class Q388WenJianDeZuiZhangJueDuiLuJing {
    public static void main(String[] args) {
        Solution solution = new Q388WenJianDeZuiZhangJueDuiLuJing().new Solution();
    }

    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public int lengthLongestPath(String input) {
            // count level path length , dfs or iter
            //dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext
            // dir
            //   tsubdir1 tfile1.ext
            //   tsubdir1
            ...
            for (int i = 0; i < path.length; i++) {
                //计算/t的个数
                String str = path[i];
                int count = countT(str);
                int parentCount = depth.getOrDefault(count, 0);
                //目录需要减去 \t的个数
                int curCount = parentCount + path[i].length() - count;
                depth.put(count + 1, curCount);
                //
                if (str.contains(".")) {
                    //说明是文件，则取最大长度
                    res = Math.max(res, curCount + count);
                }
            }
            return res;
